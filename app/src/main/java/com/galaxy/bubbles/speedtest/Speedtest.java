package com.galaxy.bubbles.speedtest;
...
public class Speedtest {
    ...
    public void setSpeedtestConfig(SpeedtestConfig c){
        synchronized (mutex){
            if(state!=0)  System.out.println("Cannot change config at this moment");
            config=c.clone();
    ...
    public void addTestPoint(TestPoint t){
        synchronized (mutex) {
            if (state == 0) state = 1;
            if (state > 1)  System.out.println("Cannot add test points at this moment");
            servers.add(t);
        }
    }
    ...
    public void selectServer(final ServerSelectedHandler callback){
        synchronized (mutex) {
            if (state == 0)  System.out.println("No test points added");
            if (state == 2)  System.out.println("Server selection is in progress");
            if (state > 2)  System.out.println("Server already selected");
            state = 2;
    ...
    public void start(final SpeedtestHandler callback){
        synchronized (mutex) {
            if (state < 3)  System.out.println("Server hasn't been selected yet");
            if (state == 4) System.out.println("Test already running");
            state = 4;
            try {
